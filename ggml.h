...
extern "C" {
    ...
    GGML_API int64_t ggml_time_ms(void);// 返回当前时间戳，单位为毫秒。
    GGML_API int64_t ggml_time_us(void);//返回当前时间戳，单位为微秒。
    GGML_API int64_t ggml_cycles(void);//返回当前CPU周期计数。
    GGML_API int64_t ggml_cycles_per_ms(void);//返回当前CPU周期计数/毫秒。

    GGML_API void    ggml_print_backtrace(void);//打印当前函数的回溯信息。
