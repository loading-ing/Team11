...
static void add_softmax_cuda(float * x,float * y, float * dst, const int ncols_x, const int nrows_x, cudaStream_t stream) {
    ...
    const int block_nums=nrows_x;
    const int block_dims=ncols_x;
...
// 使用scale对矩阵进行缩放
inline void ggml_cuda_op_qk_slsm_cublas(
    const ggml_tensor * src0, const ggml_tensor * src1, const ggml_tensor * src2, const ggml_tensor * src3, ggml_tensor * dst, const char * src0_dd_i, const float * src1_ddf_i,
    const char * src1_ddq_i, float * dst_dd_i, const int64_t row_low, const int64_t row_high, const int64_t src1_ncols,
    const int64_t src1_padded_row_size, const cudaStream_t & stream) {
    ...
    if (compute_capability >= CC_VOLTA && (src0->type == GGML_TYPE_F16 || ggml_is_quantized(src0->type)) && ggml_is_contiguous(src0) && row_diff == src0->ne[1]) {
        ...
        const half alpha_f16 = __float2half(scale);
        const half beta_f16 = 0.0f;
...
// 适配操作
static void ggml_cuda_op_qk_slsm(
    const ggml_tensor * src0, const ggml_tensor * src1,
    const ggml_tensor * src2, const ggml_tensor * src3, ggml_tensor * dst, ggml_cuda_op_qk_slsm_t op){
    ...
    for (int64_t src1_col_0 = 0; src1_col_0 < ne11; src1_col_0 += src1_col_stride) {
        ...
        for (int64_t id = 0; id < g_device_count; ++id) {
            ...
            for (int64_t i0 = 0; i0 < ne13*ne12; ++i0) {
                ...
                // do the computation
                op(src0, src1, src2, src3,dst, src0_dd_i, src1_ddf_i, src1_ddq_i, dst_dd_i,
                   row_low[id], row_high[id], src1_ncols, src1_padded_col_size, stream);
                CUDA_CHECK(cudaGetLastError());
