...
struct ggml_tensor * ggml_QK_scaled_softmax(
    ...
    GGML_ASSERT(!ggml_is_transposed(k));

    bool is_node = false;

    if (k->grad || q->grad||kq_scale->grad||kq_mask->grad) {
        is_node = true;
    }
...
// my op start
//ggml_compute_forward_qk_slsm
static void ggml_compute_forward_qk_slsm(
        const struct ggml_compute_params * params,
        const struct ggml_tensor * src0,
        const struct ggml_tensor * src1,
...
static void ggml_compute_forward(struct ggml_compute_params * params, struct ggml_tensor * tensor) {
    ...
    switch (tensor->op) {
        ...
        case GGML_OP_QK_SLSM:
            {
                ggml_compute_forward_qk_slsm(params, tensor->src[0], tensor->src[1], tensor->src[2], tensor->src[3], tensor);
            }
            break;
